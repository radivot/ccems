...
DNA="H121N"
DNA="wt"
d=subset(TK2,(dT<15)&(dC<15)&(dCTP==0)&(dTTP==0)&(ATP>=2000)&(year==2003)&(seq==DNA),select=c(ET,dT,dC,k,VdT,VdC,seq,frstAut,year))
names(d)[2:3]=c("T","C")
...
plotmod<-function(mod,d,tab,id,jj=1,kill=FALSE,pause=FALSE){
  if (class(mod)=="nls"){
    Xg <- c(0.5,0.75,1,1.5,2,3.5,5,7.5,10,15)
    if (DNA=="wt") {Cg=c(0,5,10);Tg=c(0,1,5)} else {Cg=c(0,1,10);Tg=c(0,2,10)}
...
(id="DEF.DDE:DE.DEF")
mod<-nls(k~((kt1*2*T/KT+2*kt2*T^2/(KT*KTT)+kt3*2*T*C/(KT*KTC))*It+(kc1*2*C/KC+2*kc1*C^2/(KC*KCC)+kc3*2*T*C/(KT*KTC))*Ic)/
   (1+2*T/KT+T^2/(KT*KTT)+2*T*C/(KT*KTC)+2*C/KC+C^2/(KC*KCC)), 
    start=list(kt1=.2,kt2=.4,kt3=0.1,kc1=.3,kc3=0.1,KT=5,KTT=50,KC=9,KCC=19,KTC=11),
